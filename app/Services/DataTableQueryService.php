...
class DataTableQueryService
{
	...
	static function order($request, $columns)
	{
		...
		if (isset($request['order']) && count($request['order'])) {
			$orderBy = array();
			$dtColumns = DataTableQueryService::pluck($columns, 'dt');
	 ...
	 */
	static function simple($request, $pg_details, $table, $primaryKey, $columns, $filtroAdd = NULL, $jonQuery = null, $conn = 'pgsql')
	{
		$connections = Config::get('database.connections')[$conn];
		...
		$sql_details = array(
			...
			'schema' => strpos($table, '.') ? explode(".", $table)[0] : $connections['schema']
		);

//var_dump($sql_details);

		$db = DataTableQueryService::pg_connect($sql_details);

		// Build the SQL query string from the request
		$limit = DataTableQueryService::limit($request, $columns);
		$order = DataTableQueryService::order($request, $columns);
		$innerjoin = $jonQuery;
        $where  = '';
		$where  =  DataTableQueryService::filter($request, $columns, $filtroAdd, $table);
        $select = "SELECT " . implode(", ", DataTableQueryService::pluckas($columns)) . "
		...
		$result = pg_query($db, $select) or DataTableQueryService::fatal("Error al ejecutar la consulta.\n" . pg_last_error() . "\n $select");
		...
		return array(
			...
			"data"            => DataTableQueryService::data_output($columns, $data)
		);
	}
	...
	static function pg_connect($pg_details)
	{
        $db =  pg_connect("
			...
			password={$pg_details['pass']}
			options='--search_path={$pg_details['schema']}'") or DataTableQueryService::fatal("Error en conexión a DB.\n" . pg_last_error());
		return $db;
	}
